...
public class InMemoryCurrentQuotaManagerTest {
    ...
    public void setUp() throws Exception {
        ...
        testee.setQuotaCalculator(mockedCurrentQuotaCalculator);
        testee.setMailboxManager(mockedMailboxManager);
    }
    ...
    @Test
    public void getCurrentMessageCountShouldReturnZeroWhenCalculateOnUnknownQuotaIsFalse() throws Exception {
        testee.setCalculateOnUnknownQuota(false);
        assertThat(testee.getCurrentMessageCount(QUOTA_ROOT)).isEqualTo(0);
    }

    @Test
    public void getCurrentStorageShouldReturnZeroWhenCalculateOnUnknownQuotaIsFalse() throws Exception {
        testee.setCalculateOnUnknownQuota(false);
    ...
    public void increaseShouldWork() throws Exception {
        when(mockedMailboxManager.createSystemSession((String) any(), (Logger) any())).thenAnswer(new Answer<MailboxSession>() {
            @Override
            public MailboxSession answer(InvocationOnMock invocationOnMock) throws Throwable {
                return new MockMailboxSession("user");
            }
        });
        testee.setCalculateOnUnknownQuota(false);
    ...
    public void decreaseShouldWork() throws Exception {
        ...
        testee.setCalculateOnUnknownQuota(false);
        testee.increase(QUOTA_ROOT, 10, 100);
        testee.decrease(QUOTA_ROOT, 5, 50);
        assertThat(testee.getCurrentMessageCount(QUOTA_ROOT)).isEqualTo(5);
        assertThat(testee.getCurrentStorage(QUOTA_ROOT)).isEqualTo(50);
    }
